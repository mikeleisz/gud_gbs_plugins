...
void handle_bkg_submap(UINT8 x, UINT8 y, UINT8 w, UINT8 h, INT16 offset) NONBANKED;
...
void background_submap(SCRIPT_CTX * THIS) OLDCALL BANKED {
    ...
    int16_t offset = (src_x - dest_x) + ((src_y - dest_y) * image_tile_width);

    handle_bkg_submap(dest_x, dest_y, w, h, offset);
}
...
void handle_bkg_submap(UINT8 x, UINT8 y, UINT8 w, UINT8 h, INT16 offset) NONBANKED {
    ...
    if (_is_CGB) { 
        ...
        VBK_REG = 1;
        set_bkg_submap(x, y, w, h, image_attr_ptr + offset, image_tile_width);
        VBK_REG = 0;
    }
#endif
    SWITCH_ROM(image_bank);
    set_bkg_submap(x, y, w, h, image_ptr + offset, image_tile_width);
    SWITCH_ROM(_save);
}